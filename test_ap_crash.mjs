...
async function run() {
  ...
  // Do NOT call tf.updateAppearances(font)
  // But wait, addToPage creates AP dict? Let's check.
  
  // Wait, if we manually create an empty AP dict:
  const widgets = tf.acroField.getWidgets();
  for (const w of widgets) {
    w.dict.set(PDFName.of('AP'), doc.context.obj({}));
  }
